...
parser = argparse.ArgumentParser(description='Save/Load messages from the filesystem.')
parser.add_argument('-s','--save', dest='dump', action='store_true',default=False,
                   help='Save all messages from the DB to a local directory')
parser.add_argument('-l','--load', dest='read', action='store_true',default=False,
                   help='Load all messages from a directory into the DB')
parser.add_argument('-d','--dirs', dest='dirs', action='store_true', default=False,
                   help='Store messages in directories by topic')
parser.add_argument('-t','--topic', dest='topic', action='append', default=['all'],
