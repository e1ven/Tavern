...
class Server(libtavern.utils.instancer):
    ...
    def __init__(self, slot='default'):
        ...
        self.unusedkeys = {}
        self.unusedkeys[libtavern.crypto.Usage.signing] = multiprocessing.Queue(
            self.serversettings.settings['KeyGenerator']['num_pregens'])
        self.unusedkeys[libtavern.crypto.Usage.encryption] = multiprocessing.Queue(
            self.serversettings.settings['KeyGenerator']['num_pregens'])
        ...
        self.db.safe.ensure_index('envelope', 'envelope.local.payload_sha512')
        self.db.safe.ensure_index('envelope', 'envelope.payload.author.public')
        self.db.safe.ensure_index('envelope', 'envelope.payload.author.public')
    ...
    def start(self):
        ...
        if self.serversettings.settings['defaultuser']['public_key'] is not None:
            self.defaultuser = libtavern.user.User()
            self.defaultuser.load_mongo_by_publickey(self.serversettings.settings['defaultuser']['public_key'])
            ...
            self.serversettings.settings['defaultuser']['public_key'] = self.defaultuser.Keys['master'].public
        ...
        if self.serversettings.settings['guestuser']['public_key'] is not None:
            self.guestuser = libtavern.user.User()
            self.guestuser.load_mongo_by_publickey(self.serversettings.settings['guestuser']['public_key'])
            ...
            self.serversettings.settings['guestuser']['public_key'] = self.guestuser.Keys['master'].public
        ...
        if self.serversettings.settings['serveruser']['public_key'] is not None:
            self.serveruser = libtavern.user.User()
            self.serveruser.load_mongo_by_publickey(self.serversettings.settings['serveruser']['public_key'])
            ...
            self.serversettings.settings['serveruser']['public_key'] = self.serveruser.Keys['master'].public
 ...
 #   @libtavern.utils.memorise(ttl=defaultsettings.settings['cache']['error_envelope']['seconds'], maxsize=defaultsettings.settings['cache']['error_envelope']['size'])
    def error_envelope(self, subject="Error", topic="sitecontent", body=None):
        ...
        e.payload.dict['author']['public_keys'] = "1234"
...
#    @libtavern.utils.memorise(ttl=defaultsettings.settings['cache']['formatText']['seconds'], maxsize=defaultsettings.settings['cache']['formatText']['size'])
    ...
    def get_all_user_posts(self, public_key, limit=1000):
        envelopes = []
        for envelope in self.db.safe.find('envelopes', {'envelope.local.author.public': public_key, 'envelope.payload.class': 'message'}, limit=limit, sortkey='envelope.local.time_added', sortdirection='descending'):
    ...
    def url_for(self,envelope=None,topic=None,user=None,static=None,base=True,fqdn=False):
        ...
        if static:
            ...
            if isinstance(user,libtavern.user.User):
                url =  "/~" + ''.join(user.Keys['master'].public)
            elif isinstance(user,str):
                key = libtavern.crypto.Keypair(public=public_key)
                url =  "/~" + key.public
            elif isinstance(user,libtavern.crypto.Keypair):
                url = "/~" + user.public
            else:
                raise Exception("User must be user object, or Key, or Keystr")
