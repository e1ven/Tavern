...
class lockedKey(object):
    ...
    def __init__(self,pub=None,priv=None,password=None,encryptedprivkey=None):  
        ...
        if encryptedprivkey != None:
            self.encryptedprivkey =  encryptedprivkey

        if encryptedprivkey == None and password is not None and priv is not None:
    ...
    def passkey(self,password):
        ...
        pkey =  base64.b64encode(scrypt.hash(password=password,salt=self.pubkey,N=16384)).decode('utf-8')
        return pkey
    ...
    def changepass(self,oldpasskey,newpass):
        ...
        self.encryptedprivkey = self.__encryptprivkey(password=newpass,privkey=privkey)
